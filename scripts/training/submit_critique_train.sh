...
echo "[INFO] Launching training with self-critique feedback on ${SLURM_NODELIST}"

TRAIN_MAX_TURN="${TRAIN_MAX_TURN:-5}"
EVAL_MAX_TURN="${EVAL_MAX_TURN:-5}"
MODEL_PATH="${MODEL_PATH:-Qwen/Qwen2.5-3B-Instruct}"

# Checkpoint directory: model_turns_critique_steps format
...
CKPT_DIR="/projects/bfea/lliu22/ragen_checkpoints/${MODEL_NAME}_${TRAIN_MAX_TURN}turn_critique_200steps"
mkdir -p "${CKPT_DIR}"

echo "[INFO] Model: ${MODEL_PATH}"
echo "[INFO] Max turn: ${TRAIN_MAX_TURN}"
echo "[INFO] Checkpoint dir: ${CKPT_DIR}"
echo "[INFO] Using self-critique feedback mechanism"
...
srun --ntasks=1 --cpus-per-task="${SLURM_CPUS_PER_TASK}" \
  python train.py \
    ...
    trainer.project_name=ufb_train_critique \
    trainer.experiment_name="${MODEL_NAME}_${TRAIN_MAX_TURN}turn_critique_200steps" \
