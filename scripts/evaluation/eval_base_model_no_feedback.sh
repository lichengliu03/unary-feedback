...
MODEL_NAME="${MODEL_NAME:-qwen25_3b_no_feedback}"
WANDB_PROJECT="${WANDB_PROJECT:-ragen_metamathqa_no_feedback}"
...
DATASET_TAG="MetamathQANoFeedback"  # 使用 No Feedback 环境
...
EVAL_RUN_NAME="${EVAL_RUN_NAME:-eval_no_feedback_${TIMESTAMP}}"
OUT_BASE="${RESULT_BASE_DIR:-${REPO_DIR}/result}/no_feedback_experiments"
mkdir -p "${OUT_BASE}"
RUN_OUT_DIR="${OUT_BASE}/${EVAL_RUN_NAME}"
mkdir -p "${RUN_OUT_DIR}"

echo "[INFO] Evaluating base model with NO FEEDBACK environment: ${MODEL_PATH}"
...
echo "[INFO] Using MetamathQANoFeedback environment"
...
for (( batch=0; batch<EVAL_NUM_BATCHES; batch++ )); do
  ...
  EVAL_CMD_OVERRIDES=(
    ...
    custom_envs.MetamathQANoFeedback.max_actions_per_traj=${MAX_ACTIONS_PER_TRAJ}
